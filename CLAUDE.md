...
### 2026-05-20 12:25:01 +08:00 — 手机和平板适配核查修复

本次针对六大板块在手机、平板网页端的可用性进行了核查和修复。

#### 本次完成内容

- 修复业务页在手机端可能被流程节点或固定双栏布局横向撑开的问题。
- 流程节点保持模块内横向滑动，不再把整个页面撑宽。
- 手机和平板窄屏下，表单区域强制切换为单列，避免右侧输入框被裁切。
- 顶部操作区按钮和语言下拉框允许自动换行，避免拥挤导致不可点击。
- 放大节点勾选按钮、单证复选框、单证状态下拉框等触控面积，提升手机端可点性。
- 费用/税费表格在自身区域内横向滚动，避免影响整页宽度。
- 空运、空运单证、海运单证、报关等局部板块补齐窄屏覆盖规则。

#### 验证记录

- 使用手机宽度 390px 和平板宽度 820px 对导航、外贸跟单、海运操作、空运操作、海运单证、空运单证、报关操作进行本地自动化核查。
- 已确认主要业务板块不再出现整页横向溢出。
- 已确认海运操作此前最严重的横向撑宽问题已修复。
- 已确认手机端跟单页面可打开并创建记录，页面宽度与视口一致。
- 本地自动化核查中平板跟单页曾出现一次 DOM 获取超时，单独复测手机跟单页正常；发布后仍建议在真实设备上再做一次手动点击巡检。
